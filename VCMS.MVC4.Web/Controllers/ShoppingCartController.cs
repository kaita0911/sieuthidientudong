    ...
    public class ShoppingCartController : Controller
    {
        ...
        public ActionResult Index()
        {
            using (DataContext db = new DataContext())
            {
                ...
                ViewBag.Timecho = timecho.List;
                return View(SiteConfig.ShoppingCart);
            }
        }
        public ActionResult MyOrder(string phoneNumber = "", int pageIndex = 1, int pageSize = 16)
        {
            using (DataContext db = new DataContext())
            {
                ...
                ViewBag.SearchKeyword = phoneNumber;
                var order = Order.GetByPhoneNumber(s, pageIndex, pageSize);
                return View(order);
            }
        }
        ...
        public ActionResult AddToCart(int id, int? num  , string proName = "")
        {
            using (DataContext db = new DataContext())
            {
            ...
            using (DataContext db = new DataContext())
            {
                ...
                cart.Items.ToList().ForEach(i =>
                {
                    ...
                    if (type.HasPrice && type.HasNumberProduct)
                    {
                        ...
                        {
                            bas += qty - int.Parse(i.Article.Number.ToString());

                        }
                            
                            //if(int.Parse(form["qtydu_" + i.Id.ToString()])==0)
                            //{
                            //    bas += int.Parse(form["qtydu_" + i.Id.ToString()]);
                            //}
                        else
                        {
                        }
        ...
        {
            var check = 0;
            if (SiteConfig.ShoppingCart.Items.FirstOrDefault() != null || SiteConfig.ShoppingCart.Items.FirstOrDefault().Article.ArticleType.HasNumberProduct)
            {
                foreach (var item in SiteConfig.ShoppingCart.Items.ToList())
                {
                    if (/*item.Article.Number == 0 ||*/ item.Article.IsMostView /*|| item.Quantity > item.Article.Number*/)
        ...
        public ActionResult CheckOut(FormCollection form)
        {
            using (DataContext db = new DataContext())
            {
                ...
                if (SiteConfig.ShoppingCart.Items.Count > 0)
                {
                    ...
                    var temp = SiteConfig.ShoppingCart.Items;
                    ...
                    if (form["hd_address"] != null)
                        order.Address = model.Address = form["hd_address"].ToString();
                    if (form["hd_ct"] != null)
                        order.City = int.Parse(form["hd_ct"]);
                    if (form["hd_st"] != null)
                        order.State = int.Parse(form["hd_st"]);
                    if (form["rdo_nn"] != null)
                        order.Country = model.Country = form["rdo_nn"].ToString();
                    //order.AmountShipping = model.AmountShipping = Convert.ToDecimal(ship);
                    //order.AmountShipping = model.AmountShipping = Convert.ToDecimal(total * 10 / 100);

                    //order.Ttt = pt;
                    if (model.Phone !=null)
                    {
                        var s = model.Phone.Trim();
                        while (s.IndexOf(" ") >= 0)
                            s = s.Replace(" ", "");
                        order.PhoneS = s;
                    }
